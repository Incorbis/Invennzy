...
const Overview = ({ inventory, labs, userRole = 'assistant' }) => {
  ...
  const todaysSchedule = inventory.filter(item => 
    item.nextMaintenance && isToday(item.nextMaintenance)
  ).length;
  
  const overdueItems = inventory.filter(item => 
    item.nextMaintenance && isPastDue(item.nextMaintenance)
  ).length;
  ...
  const StatCard = ({ title, value, icon: Icon, color, bgColor, action, priority }) => (
    <div className={`bg-white rounded-xl shadow-sm border-2 ${priority === 'high' ? 'border-red-200' : 'border-gray-200'} p-6 hover:shadow-md transition-all cursor-pointer`}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-gray-600">{title}</p>
          <p className="text-3xl font-bold text-gray-900 mt-2">{value}</p>
          {action && <p className="text-xs text-blue-600 mt-1 hover:underline">{action}</p>}
        </div>
  ...
  const TaskCard = ({ title, count, icon: Icon, color, urgency, onClick }) => (
    <div 
      className={`bg-white rounded-lg border-2 ${urgency === 'urgent' ? 'border-red-200 bg-red-50' : 'border-gray-200'} p-4 hover:shadow-sm transition-all cursor-pointer`}
      onClick={onClick}
    >
      <div className="flex items-center space-x-3">
        ...
        <div>
          <p className={`text-sm ${urgency === 'urgent' ? 'text-red-700' : 'text-gray-600'}`}>{title}</p>
          <p className="text-xl font-bold text-gray-900">{count}</p>
          {urgency === 'urgent' && <p className="text-xs text-red-600 font-medium">Needs Attention</p>}
        </div>
      </div>
    </div>
  );
  ...
  const LabCard = ({ lab, isAssigned }) => {
    ...
    const issueItems = labItems.filter(item => item.status === 'damaged' || item.status === 'maintenance');
    
    return (
      <div className={`p-4 border-2 ${isAssigned ? 'border-blue-200 bg-blue-50' : 'border-gray-200'} rounded-lg hover:bg-gray-50 transition-colors cursor-pointer`}>
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center space-x-2">
              <h4 className="font-medium text-gray-900">{lab.name}</h4>
              {isAssigned && <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full">Assigned</span>}
            </div>
          ...
          <div className="text-right">
            ...
            <div className="flex space-x-2 text-xs mt-1">
              <span className="text-green-600">{activeLabItems.length} Active</span>
              {issueItems.length > 0 && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 gap-3">
            {Object.entries(categoryStats).map(([category, count]) => {
              ...
              const issuesInCategory = inventory.filter(item => 
                item.category === category && (item.status === 'damaged' || item.status === 'maintenance')
              ).length;

              return (
                <div key={category} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex items-center space-x-3">
                    <div className={`p-2 rounded-lg ${categoryColors[category]}`}>
                      <Icon className="w-4 h-4 text-white" />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-900 capitalize">{category.replace('-', ' ')}</p>
                      <p className="text-xs text-gray-500">{count} total items</p>
                    </div>
                  </div>
                  <div className="text-right">
                    {issuesInCategory > 0 && (
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-4 p-3 bg-green-50 rounded-lg border-l-4 border-green-400">
            ...
            <div className="flex-1">
              <p className="text-sm text-gray-900 font-medium">Completed maintenance: Monitor #M-045</p>
              <p className="text-xs text-gray-500">Computer Lab A • 30 minutes ago</p>
            </div>
          ...
          <div className="flex items-center space-x-4 p-3 bg-blue-50 rounded-lg border-l-4 border-blue-400">
            ...
            <div className="flex-1">
              <p className="text-sm text-gray-900 font-medium">Started maintenance: Projector #P-012</p>
          ...
          <div className="flex items-center space-x-4 p-3 bg-yellow-50 rounded-lg border-l-4 border-yellow-400">
            ...
            <div className="flex-1">
              <p className="text-sm text-gray-900 font-medium">Reported issue: WiFi connectivity problems</p>
              <p className="text-xs text-gray-500">Chemistry Lab • 2 hours ago</p>
            </div>
